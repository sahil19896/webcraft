...
class WebCraftSiteEditor {
	...
	render_shell() {
		...
		this.wrapper.find("[data-theme]").on("input change", () => this.mark_dirty());
	...
	on_show() {
		...
		if (parts.length && (parts[parts.length - 1] === "theme" || parts[parts.length - 1] === "content")) {
			tab = parts.pop();
		}
	...
	render_content_sections() {
		...
		this.currentPage.sections.forEach((section, idx) => {
			...
			$block.find("[data-section-visible]").on("change", () => this.mark_dirty());
	...
	bind_content_change_handlers() {
		...
		this.$sections.find("input, textarea, select").on("input.wcEditor change.wcEditor", () => this.mark_dirty());
	}
